...
# -*- coding: utf-8 -*-
"""
Azeroth Universe - Launcher client (macOS)
========================================================

Version macOS du launcher, avec la même interface graphique que la
version Windows (Tkinter, esprit World of Warcraft : fond sombre,
dorures, barre de progression dégradée).

Ce launcher :
  1. Récupère le manifeste du client (liste des patchs, tailles et
     empreintes) depuis le même serveur que le launcher Windows.
  2. Vérifie si le client de base est déjà installé (via un fichier
     témoin) ; sinon, télécharge et extrait
     AzerothUniverse_platform_Mac.zip automatiquement.
  3. Compare le contenu local au manifeste et ne télécharge que les
     patchs manquants ou obsolètes (Data/ et Data/frFR/).

Contrairement à la version Windows, tout est unifié en un seul
manifeste (client de base + patchs) : un seul bouton "Mettre à jour"
gère aussi bien une première installation qu'une mise à jour.

Aucune dépendance externe n'est nécessaire pour l'essentiel (bibliothèque
standard Python + Tkinter). Seule l'extraction d'éventuels patchs
multi-parties (volumes RAR) nécessite un exécutable `unrar` externe
(voir find_unrar_bin ci-dessous) - le client de base (.zip) n'en a pas
besoin, il est extrait avec le module zipfile intégré à Python.

Compilation (macOS) :
    pip3 install pyinstaller
    pyinstaller --onefile --windowed --name AzerothLauncherMac \
                --icon=icon.icns azeroth_launcher_mac.py

L'exécutable généré (dist/AzerothLauncherMac.app) est prévu pour être
placé où l'utilisateur le souhaite : au premier lancement, il choisit
lui-même le dossier d'installation via le bouton "Parcourir...".
"""
...
# Même manifeste que le launcher Windows (azeroth_launcher_win.py) :
...
CLIENT_MARKER = ".client_installed"  # témoin de première installation réussie
...
MAX_RETRIES = 3                 # tentatives par fichier en cas d'échec réseau
RETRY_DELAY_SECONDS = 2         # pause entre deux tentatives (x2 à chaque essai)
...
def fetch_manifest():
    """Télécharge et parse le manifeste JSON des patchs (même source
    que le launcher Windows)."""
    req = urllib.request.Request(
...
def build_manifest():
    """
    Construit un manifeste unifié : le client de base (à extraire une
    seule fois, détecté via CLIENT_MARKER) + tous les patchs listés par
    le serveur. Contrairement au script console, il n'y a plus besoin
    de choisir manuellement entre "installation complète" et "mise à
    jour" : un seul bouton gère les deux cas selon ce qui manque.
    """
    patch_manifest = fetch_manifest()
    files = list(patch_manifest.get("files", []))

    client_size = _get_remote_size(CLIENT_ZIP_URL)
    files.insert(0, {
        "path": "AzerothUniverse_platform_Mac.zip",
        "type": "archive", "format": "zip",
        "url": CLIENT_ZIP_URL,
        "size": client_size or 0,
        "extract_to": "",
        "marker": CLIENT_MARKER,
    })

    return {
        "version": patch_manifest.get("version", "?"),
...
def set_button_enabled(btn, enabled):
    ...
    if enabled:
        btn.configure(bg=btn._bg_normal, fg=btn._fg_normal, cursor="pointinghand")
    else:
...
class LauncherApp(tk.Tk):
    ...
    def _build_titlebar(self, parent):
        ...
        close_btn = tk.Label(
            ...
            font=("Helvetica", 12), padx=12, cursor="pointinghand"
        )
